...
- It MUST be a dictionary with the following keys:
    ...
    - **const**: OPTIONAL; Fraction (String).
      Additive constant in the plane value `normal dot (x,y,z) + const`; it is not the right-hand side of `normal dot (x,y,z) = const`.
      Used when `kind` is `cut`.
    ...
    - **condition**, **lhs**, **rhs**: OPTIONAL; Dictionary.
      ...
      `condition` determines acceptance only when the plane value is exactly zero; `lhs` and `rhs` are the operands of a logical expression.
      A positive value passes and a negative value fails independently of `condition`.
      At zero, a present `condition` is evaluated recursively; otherwise `inclusive` determines acceptance.
...
``` json
{
    ...
    "properties": {
        ...
        "condition": {
            ...
            "description": "Optional boundary-only condition evaluated when the plane value is exactly zero, following this same `asu_cut` structure.",
            "properties": {
                "kind": {
                    "x-optimade-type": "string",
                    "x-optimade-unit": "inapplicable",
